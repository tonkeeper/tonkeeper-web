...
export const NftCollectionBody3: FC<{ nft: NftItemRepr }> = React.memo(
  ({ nft }) => {
    const verified = nft.approvedBy && nft.approvedBy.length > 0;
    return (
      <HeaderBody3Secondary verified={verified}>
        <TextContent>
          {nft.collection?.name ?? nft.metadata.description}
        </TextContent>
        {verified && (
          <IconBody>
            <VerificationIcon />
...
export const NftCollectionBody2: FC<{ nft: NftItemRepr }> = React.memo(
  ({ nft }) => {
    const verified = nft.approvedBy && nft.approvedBy.length > 0;
    return (
