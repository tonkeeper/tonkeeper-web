...
const getFiatValue = (coinValue: string, options: StateOptions) => {
    const price = getFiatPrice(options.stock, options.jettons, options.fiat, options.jetton);
...
export const toggleAmountState = (state: AmountState): AmountState => {
    if (state.inFiat) {
        const { primarySymbol, primaryValue, secondarySymbol, secondaryValue } = state;
        return {
