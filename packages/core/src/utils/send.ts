...
export const getJettonDecimals = (address: string, jettons: JettonsBalances): number => {
    const jetton = jettons.balances.find(item => item.jettonAddress === address);
...
export const getMaxValue = (
    ...
    const jettonInfo = jettons.balances.find(item => item.jettonAddress === jetton);
    return removeGroupSeparator(format(jettonInfo?.balance ?? 0, jettonInfo?.metadata?.decimals));
};
...
export const getRemaining = (
    ...
    if (jetton === CryptoCurrency.TON) {
        ...
        const remaining = new BigNumber(info?.balance ?? 0).minus(
            isNumeric(amount) ? new BigNumber(toNumberAmount(amount)).shiftedBy(DefaultDecimals) : 0
        );
