...
`yarn lint` runs with `--max-warnings=219`. Existing violations are demoted to `'warn'` in
...
## Demoted rules (severity baseline: 219 warnings, 0 errors)

The following rules were demoted from `error` (or `off`) to `warn`:

| Rule                          | Violations | Notes                                                                                                   |
| ----------------------------- | ---------: | ------------------------------------------------------------------------------------------------------- |
| `react-hooks/exhaustive-deps` |        219 | **Was off** — re-enabled to catch stale closures. High count is expected; treat as a backlog, not noise |
