...
export const InterceptTonLinkNotification: FC<{handleClose: (processInExtension?: boolean) => void; url: string | null}> = ({url, handleClose}) => {
  ...
  return <Notification isOpen={!!url} handleClose={handleClose}>
...
const Content: FC<{onSubmit: (form: TonLinkForm) => void}> = ({onSubmit}) => {
  ...
  return <ContentWrapper>
    ...
    <Button primary size="large" fullWidth onClick={() => onSubmit({rememberChoice, processInExtension: true})}>{t('links_interceptor_modal_continue_button')}</Button>
    <Button secondary size="large" fullWidth marginTop onClick={() => onSubmit({rememberChoice, processInExtension: false})}>{t('links_interceptor_modal_browser_button')}</Button>
    <CheckboxStyled checked={rememberChoice} onChange={setRememberChoice}>{t('links_interceptor_modal_checkbox_label')}</CheckboxStyled>
  </ContentWrapper>
}
